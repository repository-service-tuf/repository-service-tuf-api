...
ifeq ($(GITHUB_ACTION),)
	...
	docker compose run --env UMBRELLA_PATH=rstuf-umbrella --entrypoint 'bash rstuf-umbrella/tests/functional/scripts/run-ft-das.sh $(CLI_VERSION) $(PYTEST_GROUP)' --rm repository-service-tuf-api
...
ifeq ($(GITHUB_ACTION),)
	...
	docker compose run --env UMBRELLA_PATH=rstuf-umbrella --entrypoint 'bash rstuf-umbrella/tests/functional/scripts/run-ft-signed.sh $(CLI_VERSION) $(PYTEST_GROUP)' --rm repository-service-tuf-api